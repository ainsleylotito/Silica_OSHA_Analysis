...
dat |> 
  ...
  summarize(n())
```
Ok so we have 29 instances where we are missing the measurements...

We will need to exclude those in our measurements, as they are required to have a result in this instance. 
...
## Removing samples lacking unit of measurement 

Reminder: we will run a sensitivity analysis on the sample without units in the future, assuming they are coded consistent with IMIS code 9000, whcih should be in units of milligrams per cubic meter. 
...
```{r}
write_csv(dat_4, "../CleanData/study_dat.csv")
```